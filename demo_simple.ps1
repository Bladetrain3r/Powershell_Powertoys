# demo_simple.ps1
# Simple demonstration script for the new LMStudio Integration module
...
Write-Host "Simple demonstration of enhanced LMStudio module functionality`n" -ForegroundColor Yellow
...
if ($connectionResults.API.Status -eq "Connected") {
    ...
    Write-Host "Skipping text generation - API not available" -ForegroundColor Yellow
}
...
if ($connectionResults.API.Status -eq "Connected") {
    ...
    Write-Host "Skipping system prompt demo - API not available" -ForegroundColor Yellow
}
...
if (-not $SkipTTS -and $connectionResults.TTS.Status -eq "Connected") {
    ...
    try {
        ...
        Write-Host "`nAudio generated and saved to: $audioPath" -ForegroundColor Green
...
Write-Host "`n=== Demo 5: Error Handling and Input Validation ===" -ForegroundColor Magenta
...
if ($connectionResults.API.Status -eq "Connected") {
    ...
    Write-Host "Skipping error handling demo - API not available" -ForegroundColor Yellow
}

Wait-ForUser

# Demo 6: Configuration Display
Write-Host "`n=== Demo 6: Module Configuration ===" -ForegroundColor Magenta
...
if ($Interactive) {
    ...
    if ($customTest -eq 'y' -or $customTest -eq 'Y') {
        if ($connectionResults.API.Status -eq "Connected") {
            ...
            Write-Host "Custom prompts not available - API connection failed" -ForegroundColor Red
        }
    }
}
